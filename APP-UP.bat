...
:: Si hay peer pero falta connection/wallet, generarlos
if "!PEER_UP!"=="1" (
  if not exist "%ROOT%\connection.json" (
    echo [INFO] Peer activo pero falta connection.json — generando wallet...
    pushd "%ROOT%"
    node scripts\enrollAppUser.js
    set "ENROLL_RC=!ERRORLEVEL!"
    popd
    if not "!ENROLL_RC!"=="0" (
      echo [AVISO] No se pudo generar connection.json. Usando SIMULACION.
      set "PEER_UP=0"
    )
  ) else (
    rem Wallet FileSystem: appUser.id (fabric-network)
    if not exist "%ROOT%\wallet\appUser.id" if not exist "%ROOT%\wallet\appUser" (
      echo [INFO] Falta identidad wallet — regenerando...
      pushd "%ROOT%"
      node scripts\enrollAppUser.js
      popd
    )
